...
source("FILEPATH/r/get_rei_metadata.R")## for pulling data
source("FILEPATH/r/get_cause_metadata.R")## for pulling data
source("FILEPATH/r/get_location_metadata.R") ## for pulling data
...
df = read.csv("/FILEPATH/self_harm_by_firearm_asmr_2021_results.csv")
df$val <- round(df$val*100000, 1)
df_bothsex = df[df$sex_id == 3,]

listdf = lapply(list(df_bothsex), setnames, "val", "mapvar", skip_absent=TRUE)

create_breakpoints_labels = function(data) {
  # breakpoint per 10%
  #idx <- which(duplicated(round(breakpoints1,1)))
...
bkpts = lapply(list(df_bothsex), create_breakpoints_labels)

options(OutDec="\u0B7")
pdf(paste0(outdir, "Fire_arm_self_harm_ASMR.pdf"), height = 4.15, width = 7.5, pointsize = 6.5, fonts = f3)
gbd_map(data = listdf[[1]], 
        ...
        Age Standardized Rate of Death by Fire Arm Self-harm for Both Sexes, 2021",
        legend.title = "Rate per 100'000")  
