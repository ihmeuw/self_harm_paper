...
def create_percent_change_table(cause_id=718):
    ...
    df_2021 = df_2021.pivot(index=['location_name','location_id'],columns='sex', values=['val.2021','upper.2021','lower.2021'])
    ...
    # reshape to be wide on sex
    df_1990 = df_1990.pivot(index=['location_name','location_id'],columns='sex', values=['val.1990','upper.1990','lower.1990'])
    ...
    # reshape to be wide on sex
    df_change_1990_2021 = df_change_1990_2021.pivot(index=['location_name','location_id'],columns='sex', values=['val.pct_change_1990','upper.pct_change_1990','lower.pct_change_1990'])
    ...
    # reshape to be wide on sex
    df_2019 = df_2019.pivot(index=['location_name','location_id'],columns='sex', values=['val.2019','upper.2019','lower.2019'])
    ...
    # reshape to be wide on sex
    df_change_2019_2021 = df_change_2019_2021.pivot(index=['location_name','location_id'],columns='sex', values=['val.pct_change_2019','upper.pct_change_2019','lower.pct_change_2019'])

    # merge all 5 dataframes
    df = df_1990.merge(df_2021,on=['location_name','location_id']).merge(df_change_1990_2021,on=['location_name','location_id']).merge(df_2019,on=['location_name','location_id']).merge(df_change_2019_2021,on=['location_name','location_id'])
...
def main():
    ...
    df.to_csv(OUT_DIR+"appendix_table_1_asmr_change_1990_2019_2021_country.csv", index=False)
